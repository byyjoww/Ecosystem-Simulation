...
public class SwipeInputController : MonoBehaviour
{
    ...
    float swipeTime = Int32.MaxValue;
    ...
    public static Vector2 ScreenToWorldCoordinates(Vector2 coords)
    {
        return Camera.main.ScreenToWorldPoint(new Vector3(coords.x, coords.y));
    }
    #endregion

    #region OUTPUT
    [Header("Quadrant Events")]
    [SerializeField] private SwipeScriptableEvent OnSingleSwipe;
    [SerializeField] private SwipeScriptableEvent OnDoubleSwipe;
    public Action<Vector2, Vector2, TouchPhase> OnSingleSwipeDetails;
    public Action<Vector2, Vector2, TouchPhase> OnDoubleSwipeDetails;
    ...
    void InvokeSwipeEvents(SwipeType type, Vector2 pos, Vector2 delta, TouchPhase phase)
    {
        if (type == SwipeType.Single)
        {
            ...
            if (phase == TouchPhase.Ended)
            {
                ...
                if (OnSingleSwipe) OnSingleSwipe.Raise(pos, delta, InQuadrant(delta));
                else Debug.LogError("OnSingleSwipe not assigned.");
            }            
        }
        ...
        if (type == SwipeType.Double)
        {
            ...
            if (phase == TouchPhase.Ended)
            {
                ...
                if (OnDoubleSwipe) OnDoubleSwipe.Raise(pos, delta, InQuadrant(delta));
                else Debug.LogError("OnDoubleSwipe not assigned.");
            }  
        }
    }
